...
/** Conflito transitório do Postgres serializável (dois cadastros quase simultâneos). */
...
/** Repete a operação em conflito de serialização (transitório); propaga os demais erros. */
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (nameRaw && (!firstName || !lastName)) {
      ...
      if (!lastName)
        lastName =
          parts.slice(1).join(" ") || (firstName ? `${firstName} (trial)` : "");
    }
    ...
    let packageTier =
      body?.packageTier != null
        ? Number(body.packageTier)
        : segment === "LONG_TERM"
          ? TRIAL_LT_DEFAULT.packageTier
          : null;
    let avgRent =
      body?.avgRent != null
        ? Number(body.avgRent)
        : segment === "LONG_TERM"
          ? TRIAL_LT_DEFAULT.avgRent
          : null;
    let unitCount =
      body?.unitCount != null
        ? Number(body.unitCount)
        : segment === "LONG_TERM"
          ? TRIAL_LT_DEFAULT.unitCount
          : null;
    ...
    if (existing) {
      return NextResponse.json(
        { ok: false, error: "Email ja existe." },
        { status: 409 },
      );
    }
        ...
        async (tx) => {
          ...
          const subscription = await tx.subscription.create({
            data: {
              ...
              interval,
              avgRent:
                avgRent != null ? new Prisma.Decimal(avgRent) : undefined,
