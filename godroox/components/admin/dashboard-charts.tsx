...
export function ChartProjecoesAtuais({ data }: { data: ProjecaoMesItem[] }) {
  const maxVal = Math.max(
    ...data.map((d) => d.atual + d.projecao),
    1
  );

  return (
    <Card className="border-0 bg-secondary-800 overflow-hidden">
      ...
      <CardContent className="pt-0">
          ...
          {data.map((d) => {
            ...
            return (
              <div key={d.month} className="flex flex-1 flex-col items-center gap-0.5">
                <div
                  className="flex w-full flex-col-reverse rounded-t overflow-hidden"
                  style={{ height: `${pct}%`, minHeight: 28 }}
                >
                  <div
                    className="w-full min-h-[2px] rounded-t bg-primary-400"
                    style={{ height: `${hProj}%` }}
                  />
                  <div
                    className="w-full min-h-[2px] rounded-t bg-blue-500"
                    style={{ height: `${hAtual}%` }}
                  />
                </div>
        ...
        <div className="mt-2 flex justify-center gap-4 text-xs text-white/70">
          <span className="flex items-center gap-1.5">
            <span className="h-2.5 w-3 rounded bg-blue-500" /> Atual
          </span>
          <span className="flex items-center gap-1.5">
            <span className="h-2.5 w-3 rounded bg-primary-400" /> Projeção
          </span>
        </div>
      </CardContent>
    </Card>
      ...
      <CardHeader className="pb-2">
        ...
        <div className="flex flex-wrap gap-4 text-xs">
          <span className="flex items-center gap-1.5">
            ...
            Semana atual: R$ {semanaAtual.toLocaleString('pt-BR')}
          </span>
          <span className="flex items-center gap-1.5">
            <span className="h-2 w-2 rounded-full bg-primary-400" />
            Semana anterior: R$ {semanaAnterior.toLocaleString('pt-BR')}
          </span>
        </div>
      </CardHeader>
      <CardContent className="pt-0">
            ...
            <div key={d.label} className="flex flex-1 flex-col items-center">
              <div className="flex w-full gap-0.5">
                <div
                  ...
                  style={{ height: `${(d.atual / maxV) * 100}px`, minHeight: 2 }}
                />
                <div
                  className="flex-1 rounded-t bg-primary-400/80"
                  style={{ height: `${(d.anterior / maxV) * 100}px`, minHeight: 2 }}
                />
              </div>
    ...
    <Card className="border-secondary-200">
      ...
      <CardContent className="pt-0">
        <div className="flex flex-col items-center gap-4 sm:flex-row">
          <div
            className="h-32 w-32 shrink-0 rounded-full border-[10px] border-secondary-100"
