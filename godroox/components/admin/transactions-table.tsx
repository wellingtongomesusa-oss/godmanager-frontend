...
const TIPO_OPTIONS: { value: TipoDemanda | ''; label: string }[] = [
  ...
  ...(Object.entries(TIPO_DEMANDA_LABELS) as [TipoDemanda, string][]).map(
        ...
        <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFilter('status', e.target.value as TransactionStatus | '')
              }
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFilter('tipoDemanda', e.target.value as TipoDemanda | '')
              }
              className="flex h-11 w-full rounded-lg border border-secondary-300 bg-white px-4 py-2 text-sm text-secondary-900 focus:outline-none focus:ring-2 focus:ring-primary-500"
            >
              {TIPO_OPTIONS.map((o) => (
          ...
          <div>
            ...
            <select
              value={filters.pais ?? ''}
              onChange={(e) => setFilter('pais', e.target.value || undefined)}
              className="flex h-11 w-full rounded-lg border border-secondary-300 bg-white px-4 py-2 text-sm text-secondary-900 focus:outline-none focus:ring-2 focus:ring-primary-500"
            >
              {PAIS_OPTIONS.map((o) => (
                <option key={o.value} value={o.value}>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setFilter('dataInicio', e.target.value || undefined)
              }
              className="flex h-11 w-full rounded-lg border border-secondary-300 bg-white px-4 py-2 text-sm text-secondary-900 focus:outline-none focus:ring-2 focus:ring-primary-500"
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setFilter('dataFim', e.target.value || undefined)
              }
              className="flex h-11 w-full rounded-lg border border-secondary-300 bg-white px-4 py-2 text-sm text-secondary-900 focus:outline-none focus:ring-2 focus:ring-primary-500"
            />
          </div>
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-secondary-200 bg-secondary-50">
                <th className="px-4 py-3 font-semibold text-secondary-900">
                  Cliente
                </th>
                <th className="px-4 py-3 font-semibold text-secondary-900">
                  Tipo de demanda
            ...
            <tbody>
              {transactions.length === 0 ? (
                ...
                transactions.map((t) => (
                  <tr
                    key={t.id}
                    className="border-b border-secondary-100 hover:bg-secondary-50/80"
                  >
                    <td className="px-4 py-3 text-secondary-700">{t.cliente}</td>
                    <td className="px-4 py-3 text-secondary-700">
                      {TIPO_DEMANDA_LABELS[t.tipoDemanda]}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={cn(
                          'inline-flex rounded-full px-2 py-1 text-xs font-medium',
                          approvalLabelClass(t)
                        )}
                      >
                        {getApprovalDisplayLabel(t)}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap items-center gap-1">
                        {t.status !== 'cancelada' && (
                          <>
                            {t.statusAprovador1 === 'pendente' && (
                              <Button
                                size="sm"
                                variant="outline"
                                className="text-xs"
                                onClick={() => onApprove1(t.id)}
                              >
                                1ª aprovação
                              </Button>
                            )}
                            {t.statusAprovador1 === 'aprovado' &&
                              t.statusAprovador2 === 'pendente' && (
                                <Button
                                  size="sm"
                                  variant="outline"
                                  className="text-xs"
                                  onClick={() => onApprove2(t.id)}
                                >
                                  2ª aprovação
                                </Button>
                              )}
                        ...
                        <Button
                          ...
                          className={cn(
                            'text-xs',
                            t.status === 'feito' && 'bg-success-100 text-success-700'
                          )}
                          onClick={() => onStatusChange(t.id, 'feito')}
                        >
                          Feito
                        </Button>
                        <Button
                          ...
                          onClick={() => onStatusChange(t.id, 'aguardando')}
                        >
                          Aguardando
                        </Button>
                        <Button
                          ...
                          variant="ghost"
                          className={cn(
                            'text-xs text-danger-600 hover:bg-danger-50',
                            t.status === 'cancelada' && 'bg-danger-100'
                          )}
                          onClick={() => onStatusChange(t.id, 'cancelada')}
                        >
                          Cancelada
                        </Button>
                      </div>
