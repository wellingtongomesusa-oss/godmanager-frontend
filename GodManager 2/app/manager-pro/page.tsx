...
type KpiSection = { title: Bilingual; items: KpiDef[]; featured?: boolean };

/** Um cartão por destino do menu (inclui sub-rotas), alinhado a `MASTER_ONE_NAV` */
const HOME_KPI_SECTIONS: KpiSection[] = [
  {
    title: { en: 'GodManager Trust', pt: 'GodManager Trust' },
    featured: true,
    items: [
      {
        label: { en: 'GodManager Trust · Rent roll', pt: 'GodManager Trust · Rent roll' },
        value: '1.248',
        moduleName: { en: 'GodManager Trust', pt: 'GodManager Trust' },
        detail: {
          en: 'Menu: GodManager Trust ▸ Rent roll · units & tenants',
          pt: 'Menu: GodManager Trust ▸ Rent roll · unidades e inquilinos',
        },
      ...
      {
        label: { en: 'GodManager Trust (LT)', pt: 'GodManager Trust (LT)' },
        value: '96',
        moduleName: { en: 'GodManager Trust', pt: 'GodManager Trust' },
        detail: {
          en: 'Menu: GodManager Trust ▸ LT · long-term module',
          pt: 'Menu: GodManager Trust ▸ LT · módulo longo prazo',
        },
    ...
    <div>
            ...
            {locale === 'pt' ? (
              <>
                ...
                <code className="mt-1 block whitespace-pre-wrap rounded bg-white p-2 text-xs">
                  cd …/cursor-projects/GodManager 2{'\n'}
                  rm -rf .next{'\n'}
                  npm run dev
                </code>
              ...
              <>
                ...
                <strong>cursor-projects/GodManager 2</strong>, delete <code>.next</code>, and{' '}
      ...
      {HOME_KPI_SECTIONS.map((section) => (
        <section
          ...
          className={`mt-8 ${section.featured ? 'rounded-2xl border-2 border-[#c47b28]/80 bg-gradient-to-br from-[#fff7ed] via-[#faf8f5] to-[#ecfdf5] p-4 shadow-[0_8px_30px_rgba(196,123,40,0.15)]' : ''}`}
        >
          <p
            className={`text-[10px] font-semibold uppercase tracking-[0.14em] ${section.featured ? 'text-[#9a3412]' : 'text-[var(--ink3)]'}`}
          >
            {pick(section.title, locale)}
            {section.featured ? (
              <span className="ml-2 rounded-full bg-[#c47b28] px-2 py-0.5 text-[9px] font-bold tracking-normal text-white">
